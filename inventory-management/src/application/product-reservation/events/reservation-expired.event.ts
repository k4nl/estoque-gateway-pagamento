...
export class ReservationExpiredEvent {
  ...
  async execute(external_id: string) {
    ...
    const product_reservation = product.expireReservation({
      product_reservation: reservation,
      batch,
    });

    await this.reserveProductRepository.expireReservation(
      product_reservation,
      batch,
    );

    return {
      message: `Reservation ${external_id} expired successfully`,
    };
  }
}
